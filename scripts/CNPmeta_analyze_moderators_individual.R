...
nadd_leafnp_photo_summary <- data.frame(trait = "leaf_np", 
                                       nut_add = "n",
                                       mod_results(nadd_leafnp_photo, 
                                                   mod = "photo_path", 
                                                   group = "exp")$mod_table,
                                       z = coef(summary(nadd_leafnp_photo))[2,3],
                                       p = coef(summary(nadd_leafnp_photo))[2, 4]) %>%
  dplyr::select(trait, nut_add, photo = name, estimate, z, p, lowerCL, upperCL)

# P addition
padd_leafnp_photo <- rma.mv(logr, 
                           logr_var,
                           method = "REML", 
                           random = ~ 1 | exp, 
                           mods = ~ photo_path,
...
padd_leafnp_photo_summary <- data.frame(trait = "leaf_np", 
                                       nut_add = "p",
                                       mod_results(padd_leafnp_photo, 
                                                   mod = "photo_path", 
                                                   group = "exp")$mod_table,
                                       z = coef(summary(padd_leafnp_photo))[2,3],
                                       p = coef(summary(padd_leafnp_photo))[2, 4]) %>%
  dplyr::select(trait, nut_add, photo = name, estimate, z, p, lowerCL, upperCL)

# N+P addition
npadd_leafnp_photo <- rma.mv(logr, 
                            logr_var,
                            method = "REML", 
                            random = ~ 1 | exp, 
                            mods = ~ photo_path,
...
npadd_leafnp_photo_summary <- data.frame(trait = "leaf_np", 
                                        nut_add = "np",
                                        mod_results(npadd_leafnp_photo, 
                                                    mod = "photo_path", 
                                                    group = "exp")$mod_table,
                                        z = coef(summary(npadd_leafnp_photo))[2,3],
                                        p = coef(summary(npadd_leafnp_photo))[2, 4]) %>%
  dplyr::select(trait, nut_add, photo = name, estimate, z, p, lowerCL, upperCL)

## Merge leaf N:P moderator results, with some light cleaning
leafnp_photo_summary <- rbind(nadd_leafnp_photo_summary, 
                             padd_leafnp_photo_summary, 
                             npadd_leafnp_photo_summary) %>%
  mutate(estimate = round(estimate, digits = 3),
         across(z:upperCL, ~ round(.x, digits = 3)),
         p = as.character(ifelse(p < 0.001, "<0.001", p)))
...
marea_photo_summary %>%
  ...
  full_join(leafnp_photo_summary) %>%
...
nadd_leafnp_nfix_summary <- data.frame(trait = "leaf_np", 
                                      nut_add = "n",
                                      mod_results(nadd_leafnp_nfix, 
                                                  mod = "n_fixer", 
                                                  group = "exp")$mod_table,
                                      z = coef(summary(nadd_leafnp_nfix))[2,3],
                                      p = coef(summary(nadd_leafnp_nfix))[2, 4]) %>%
  dplyr::select(trait, nut_add, nfix = name, estimate, z, p, lowerCL, upperCL)

# P addition
padd_leafnp_nfix <- rma.mv(logr, 
                          logr_var,
                          method = "REML", 
                          random = ~ 1 | exp, 
                          mods = ~ n_fixer,
...
padd_leafnp_nfix_summary <- data.frame(trait = "leaf_np", 
                                      nut_add = "p",
                                      mod_results(padd_leafnp_nfix, 
                                                  mod = "n_fixer", 
                                                  group = "exp")$mod_table,
                                      z = coef(summary(padd_leafnp_nfix))[2,3],
                                      p = coef(summary(padd_leafnp_nfix))[2, 4]) %>%
  dplyr::select(trait, nut_add, nfix = name, estimate, z, p, lowerCL, upperCL)

# N+P addition
npadd_leafnp_nfix <- rma.mv(logr, 
                           logr_var,
                           method = "REML", 
                           random = ~ 1 | exp, 
                           mods = ~ n_fixer,
...
npadd_leafnp_nfix_summary <- data.frame(trait = "leaf_np", 
                                       nut_add = "np",
                                       mod_results(npadd_leafnp_nfix, 
                                                   mod = "n_fixer", 
                                                   group = "exp")$mod_table,
                                       z = coef(summary(npadd_leafnp_nfix))[2,3],
                                       p = coef(summary(npadd_leafnp_nfix))[2, 4]) %>%
  dplyr::select(trait, nut_add, nfix = name, estimate, z, p, lowerCL, upperCL)

## Merge Parea moderator results, with some light cleaning
leafnp_nfix_summary <- rbind(nadd_leafnp_nfix_summary, 
                            padd_leafnp_nfix_summary, 
                            npadd_leafnp_nfix_summary) %>%
  mutate(estimate = round(estimate, digits = 3),
         across(z:upperCL, ~ round(.x, digits = 3)),
         p = as.character(ifelse(p < 0.001, "<0.001", p)))
...
nadd_leafnp_myc_summary <- data.frame(trait = "leaf_np", 
                                     nut_add = "n",
                                     mod_results(nadd_leafnp_myc, 
                                                 mod = "myc_nas", 
                                                 group = "exp")$mod_table,
                                     z = coef(summary(nadd_leafnp_myc))[2,3],
                                     p = coef(summary(nadd_leafnp_myc))[2, 4]) %>%
  dplyr::select(trait, nut_add, myc = name, estimate, z, p, lowerCL, upperCL)

# P addition
padd_leafnp_myc <- rma.mv(logr, 
                         logr_var,
                         method = "REML", 
                         random = ~ 1 | exp, 
                         mods = ~ myc_nas,
...
padd_leafnp_myc_summary <- data.frame(trait = "leaf_np", 
                                     nut_add = "p",
                                     mod_results(padd_leafnp_myc, 
                                                 mod = "myc_nas", 
                                                 group = "exp")$mod_table,
                                     z = coef(summary(padd_leafnp_myc))[2,3],
                                     p = coef(summary(padd_leafnp_myc))[2, 4]) %>%
  dplyr::select(trait, nut_add, myc = name, estimate, z, p, lowerCL, upperCL)

# N+P addition
npadd_leafnp_myc <- rma.mv(logr, 
                          logr_var,
                          method = "REML", 
                          random = ~ 1 | exp, 
                          mods = ~ myc_nas,
...
npadd_leafnp_myc_summary <- data.frame(trait = "leaf_np", 
                                      nut_add = "np",
                                      mod_results(npadd_leafnp_myc, 
                                                  mod = "myc_nas", 
                                                  group = "exp")$mod_table,
                                      z = coef(summary(npadd_leafnp_myc))[2,3],
