# analyse_meta, script by Beni Stocker from NWG paper
...
# analyse_meta_spp, function adapted by Evan Perkowski
# from analyse_meta from Beni Stocker above to include
# species identity traits as moderator variables
analyse_meta_spp <- function(df, nam_target){
  ...
  if (nrow(filter(df, var == !!nam_target)) < 3){
    ...
    modl <- try(metafor::rma.mv( 
      ...
      control = list(stepadj = 0.3), 
      data = df %>% 
        filter(var == !!nam_target & aggregation == "species")
    ))
